...
class FlatSchemas:
    ...
    IMAGE_FILE = 'flat_image_schema.yaml'
    
    def get_flat_meta_schema(self) -> dict[str, typing.Any]:        
        """
    ...
    def get_flat_image_schema(self) -> dict[str, typing.Any]:        
        """
        Definites the metadata (fits headers) associated with flat image
        ...
        image_dict['flat'] = flat_schema
        
        return image_dict

    
    def validate_flat_image_schema(self, data_dict):        
        """
        ...
        if valid is False:
            print('Flat meta data did not validate')
            return False

        
        # The resolver maps schema URIs (like 'flat_schema.yaml') to their content
        resolver = RefResolver(
            base_uri='flat_image_schema.yaml',  # The base URI of the main schema
    ...
    def get_flat_schema(self) -> dict[str, typing.Any]:        
        """
        """
        meta_dict = self.get_flat_meta_schema()
        
        image_dict = self.get_flat_image_schema()

        flat = {}
        flat['meta'] = meta_dict
        flat['image'] = image_dict
        return flat
